function [ H , inlier_num ]= RANSACFit(p1, p2, match, maxIter, seedSetSize, maxInlierError, goodFitThresh )
...
for i = 1 : maxIter,
    ...
    %     eta = affine_leastsquare(p1(beta(:, 1), :), p2(beta(:, 2), :));
    ...
    if sum(epsilon(:)) + alpha >= goodFitThresh,
        ...
        %         eta = affine_leastsquare(p1(zeta(:, 1), :), p2(zeta(:, 2), :));
        eta = ComputeAffineMatrix(p1(zeta(:, 1), :), p2(zeta(:, 2), :));
        dist = ComputeError(eta, p1, p2, zeta);
        theta = sum(dist);
        if theta < iota,
            H = eta;
            kappa = lambda;
            dist_thres = theta/length(zeta);
            inlier_num = sum( dist<dist_thres );
            iota = theta;
...
affine_pt1 = H*[ pt1(match(:, 1),:)' ;  ones( 1 , size( match, 1) )];
affine_pt1 = [ affine_pt1( 1, : )./ affine_pt1( 3, : ) ; affine_pt1( 2, : )./ affine_pt1( 3, : )];
diff = affine_pt1 - pt2(match(:, 2),:)' ;
