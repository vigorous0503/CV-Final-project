...
if currentFrameIndex < refFrameIndex
    num = refFrameIndex - currentFrameIndex;
    if num == 1
        T = i_To_iPlusOne_Transform{currentFrameIndex};
    else
        for i = 1 :  num-1
            T =  i_To_iPlusOne_Transform{currentFrameIndex+i}* i_To_iPlusOne_Transform{currentFrameIndex};
            i_To_iPlusOne_Transform{currentFrameIndex} = T;
        end
    end
    
elseif currentFrameIndex > refFrameIndex
    num = currentFrameIndex - refFrameIndex;
    if num == 1
        T = pinv( i_To_iPlusOne_Transform{refFrameIndex} );
    else
        a = pinv( i_To_iPlusOne_Transform{refFrameIndex});
        for i = 1 :  num-1
            T =  a* pinv( i_To_iPlusOne_Transform{refFrameIndex+i} );
            a = T;
        end
    end
end
